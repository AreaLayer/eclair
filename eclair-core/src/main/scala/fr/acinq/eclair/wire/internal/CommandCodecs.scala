...
import fr.acinq.eclair.wire.protocol.LightningMessageCodecs.channelFlagsCodec
...
object CommandCodecs {
  ...
  private val legacyFailureMessageCodec = discriminated[FailureMessage].by(uint16)
    ...
    .typecase(UPDATE | 20, (("messageFlags" | byte) :: channelFlagsCodec :: ("channelUpdate" | channelUpdateWithLengthCodec) :: ("tlvs" | provide(TlvStream.empty[FailureMessageTlv]))).as[ChannelDisabled])
