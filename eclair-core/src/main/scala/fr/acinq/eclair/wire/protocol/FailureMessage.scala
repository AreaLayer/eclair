...
case class AmountBelowMinimum(amount: MilliSatoshi, update: ChannelUpdate, tlvs: TlvStream[FailureMessageTlv] = TlvStream.empty) extends Update { def message = s"payment amount was below the minimum required by the channel" }
case class FeeInsufficient(amount: MilliSatoshi, update: ChannelUpdate, tlvs: TlvStream[FailureMessageTlv] = TlvStream.empty) extends Update { def message = s"payment fee was below the minimum required by the channel" }
...
case class ChannelDisabled(messageFlags: Byte, channelFlags: ChannelUpdate.ChannelFlags, update: ChannelUpdate, tlvs: TlvStream[FailureMessageTlv] = TlvStream.empty) extends Update { def message = "channel is currently disabled" }
...
object FailureMessageCodecs {
  ...
  val failureMessageCodec = discriminatorWithDefault(
    discriminated[FailureMessage].by(uint16)
      ...
      .typecase(UPDATE | 20, (("messageFlags" | byte) :: channelFlagsCodec :: ("channelUpdate" | channelUpdateWithLengthCodec) :: ("tlvs" | failureTlvsCodec)).as[ChannelDisabled])
