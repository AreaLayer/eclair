...
private class ReplaceableTxFunder(nodeParams: NodeParams,
  ...
  def signWalletInputs(locallySignedTx: ReplaceableTxWithWalletInputs, txFeerate: FeeratePerKw, amountIn: Satoshi): Behavior[Command] = {
    ...
    locallySignedTx match {
      case ClaimLocalAnchorWithWitnessData(anchorTx) =>
        val psbt = new Psbt(anchorTx.tx)
        val updated = psbt.updateWitnessInput(anchorTx.input.outPoint, anchorTx.input.txOut, null, fr.acinq.bitcoin.Script.parse(anchorTx.input.redeemScript), null, java.util.Map.of())
        val signed = EitherKt.flatMap(updated, (psbt: Psbt) => psbt.finalizeWitnessInput(0, anchorTx.tx.txIn.head.witness))
        if (signed.isLeft) {
          SignWalletInputsFailed(new RuntimeException(signed.getLeft.toString))
        } else {
          context.pipeToSelf(bitcoinClient.processPsbt(signed.getRight)) {
            case Success(processPsbtResponse) =>
              val extracted = processPsbtResponse.psbt.extract()
              if (extracted.isLeft) SignWalletInputsFailed(new RuntimeException(extracted.getLeft.toString)) else SignWalletInputsOk(extracted.getRight)
            case Failure(reason) => SignWalletInputsFailed(reason)
          }
        }

      case htlcTx: HtlcWithWitnessData =>
        val psbt = new Psbt(htlcTx.txInfo.tx)
        val updated = psbt.updateWitnessInput(htlcTx.txInfo.input.outPoint, htlcTx.txInfo.input.txOut, null, fr.acinq.bitcoin.Script.parse(htlcTx.txInfo.input.redeemScript), null, java.util.Map.of())
        val signed = EitherKt.flatMap(updated, (psbt: Psbt) => psbt.finalizeWitnessInput(0, htlcTx.txInfo.tx.txIn.head.witness))
        if (signed.isLeft) {
          SignWalletInputsFailed(new RuntimeException(signed.getLeft.toString))
        } else {
          context.pipeToSelf(bitcoinClient.processPsbt(signed.getRight)) {
